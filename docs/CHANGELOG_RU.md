## 1.5.1
- Изменение возвращаемого типа в методе `SendMessageAsync`: `string` -> `MessageResponse`

##  1.5.0
- Добавлен метод для завершения генерации  
- Добавлен метод удаления сессии чата  

## 1.4.1
- Исправлена ошибка, из-за которой инициализация DeepSeekAPI с некоторыми токенами приводила к сбою  
- Исправлена критическая ошибка, из-за которой сообщения могли не отправляться  

## 1.4.0
- Добавлен метод SendMessageAsync с поддержкой stream-ответа
- Добавлена поддержка CancellationToken во всех публичных методах клиента
- Добавлена поддержка HTTP Proxy
- Вынесена работа с HTTP в отдельный проект APIEngine
- Переработана архитектура клиента и разделение ответственности между API-слоем и HTTP-слоем
- Удалены лишние и дублирующиеся HTTP заголовки в запросах

## 1.3.1
- Добавлена поддержка режима распознования (`ModelType.Vision`) через `ChatSettings`
